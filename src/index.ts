...
export type {
  AddImageLayersRequest,
  AddImageLayersResult,
  ImageDocumentContract,
  ImageLayerContract,
  ImageLayerId
} from "./contracts";
...
type DrawableImage = ImageBitmap | HTMLImageElement | HTMLCanvasElement | OffscreenCanvas;
...
const defaultExport: Required<ExportOptions> = {
  type: "image/png",
  quality: 0.92
};

export class ImageKit {
  private constructor(private canvas: CanvasLike) {}

  static async from(source: ImageSource): Promise<ImageKit> {
  ...
  async toFile(options: FileExportOptions = {}): Promise<File> {
...
async function loadDrawable(source: ImageSource): Promise<DrawableImage> {
...
function loadImageElement(src: string, revokeAfterLoad = false): Promise<HTMLImageElement> {
...
function readWidth(source: DrawableImage): number {
...
function readHeight(source: DrawableImage): number {
...
function resized(width: number, height: number, sourceWidth: number, sourceHeight: number) {
  return {
...
function assertPositiveRect(rect: CropRect): void {
...
function closeIfBitmap(source: DrawableImage): void {
