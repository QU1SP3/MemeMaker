...
import { backgroundPresets, defaultBackgroundPreset } from "./backgrounds";
import { canvasPresets, defaultCanvasPreset, findCanvasPreset } from "./canvas-presets";
import { icons } from "./icons";
import { MAX_IMAGE_LAYERS } from "./contracts";
import "./demo.css";
const fileInput = query("#file");
const canvasPresetSelect = query("#canvas-preset");
const textToolbar = query("#text-toolbar");
const textContentInput = query("#text-content");
const textSizeSelect = query("#text-size");
const textColorInput = query("#text-color");
const canvas = query("#canvas");
const preview = query("#preview");
const layersList = query("#layers");
const backgroundsList = query("#backgrounds");
const layerCount = query("#layer-count");
const layerUp = query("#layer-up");
const layerDown = query("#layer-down");
const layerDelete = query("#layer-delete");
const buttons = {
    rotate: query("#rotate"),
    flip: query("#flip"),
    lasso: query("#lasso"),
    text: query("#text"),
    textUppercase: query("#text-uppercase"),
    textAlignLeft: query("#text-align-left"),
    textAlignCenter: query("#text-align-center"),
    textAlignRight: query("#text-align-right"),
...
function syncPreviewCanvasSize() {
    ...
    const scale = Math.min(panelWidth / documentContract.canvas.width, panelHeight / documentContract.canvas.height);
...
function createLayer(file, image) {
    const scale = Math.min(documentContract.canvas.width / image.width, documentContract.canvas.height / image.height, 1);
...
async function exportDocumentBlob() {
    ...
    for (const layer of [...layers].reverse()) {
        ...
        context.globalAlpha = layer.opacity;
        if (layer.type === "image") {
            context.drawImage(layer.image.toCanvas(), layer.transform.x, layer.transform.y, layer.width * layer.transform.scaleX, layer.height * layer.transform.scaleY);
        }
        else {
...
function renderBackgroundList() {
    backgroundsList.replaceChildren(...backgroundPresets.map((background) => {
        const item = document.createElement("button");
...
async function setDocumentBackground(background) {
...
function setupCanvasPresetSelect() {
...
function syncLayerOrderButtons() {
...
function createExtractedLayer(sourceLayer, image, output, bounds) {
    return {
        id: crypto.randomUUID(),
        type: "image",
...
function drawLassoPath(context) {
...
function drawTextLayer(context, layer, x, y, viewportScale) {
...
function drawActiveLayerSelection(context) {
...
function drawCanvasFrame(context) {
    if (!viewportState) {
        return;
    }
    context.save();
    ...
    context.lineWidth = 1;
    drawDocumentBackground(context, viewportState.bounds.width * viewportState.scale, viewportState.bounds.height * viewportState.scale, viewportState.offsetX, viewportState.offsetY);
    context.strokeRect(viewportState.offsetX, viewportState.offsetY, viewportState.bounds.width * viewportState.scale, viewportState.bounds.height * viewportState.scale);
    context.restore();
}
function drawDocumentBackground(context, width, height, x = 0, y = 0) {
    context.fillStyle = "#ffffff";
...
function hitTestResizeHandle(event) {
...
function resizeActiveLayer(event) {
...
function hitTestMoveHandle(event) {
    const rect = getActiveLayerScreenRect();
    if (!rect) {
        return false;
    }
...
function getResizeHitHandles(rect) {
...
function drawMoveHandle(context, rect) {
    const handle = getMoveHandle(rect);
...
function cursorForHit(moveHit, resizeHandle) {
    if (moveHit) {
        return "move";
    }
...
function syncTextLayerSize(layer) {
    const context = canvas.getContext("2d");
    if (!context) {
        return;
    }
...
function resolveTextX(layer, x, width) {
...
function syncDocumentContract() {
    documentContract.layers = layers.map(layerToContract);
}
function layerToContract(layer) {
    const base = {
        id: layer.id,
        type: layer.type,
        name: layer.name,
